...
dados <- read.csv2("MOS2D12160405 (2021-07-28)60sec.csv", stringsAsFactors = FALSE, dec = ",", sep = ",", header = TRUE)
...
# Filtrando os dados para excluir os dias 
dados <- dados %>%
  filter(!(day(Date) %in% c(12, 13, 14, 15, 25, 26, 27, 28)))
...
# Gerando o gráfico com ajustes para melhorar a visualização
grafico <- ggplot(dados, aes(x = DateTime, y = Vector.Magnitude, color = as.factor(Date))) + 
  geom_line(linewidth = 0.8, na.rm = TRUE) +  # Usa linewidth menor para reduzir a sobrecarga
  ...
  scale_y_continuous(
    expand = c(0, 0)  # Remove o excesso de espaço no eixo Y
  ) +
